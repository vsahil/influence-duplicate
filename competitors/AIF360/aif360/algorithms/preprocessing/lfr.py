...
class LFR(Transformer):
    ...
    def transform(self, dataset, threshold=0.5, **kwargs):
        ...
        transformed_features = np.zeros(shape=np.shape(dataset.features))       # this puts the zeros 
        transformed_labels = np.zeros(shape=np.shape(dataset.labels))          # this puts the zeros
