...
class GermanDataset(StandardDataset):
    ...
    def __init__(self, label_name='credit', favorable_classes=[1],
                 ...
                 na_values=[], custom_preprocessing=my_preprocessing,
                 metadata=default_mappings, permute=-1):
        ...
        try:
            ...
            if permute == -1:
                ...
                new1 = df_ordered.sort_values(by=['credit_amount']).reset_index(drop=True)
                new2 = df.sort_values(by=['credit_amount']).reset_index(drop=True)
                z = new1 == new2
                assert(sum([z[i].unique()[0] for i in z.columns.tolist()]) == len(z.columns.tolist()))
        ...
        # import ipdb; ipdb.set_trace()
        # print("hello")
        super(GermanDataset, self).__init__(df=df_ordered, label_name=label_name,
