...
from load_german_credit import before_massaging_dataset, massaged_dataset
...
model_ = Fully_Connected(
    ...
    log_dir=f'throw/log{model_count}',
    hvp_files = f"inverse_HVP_schm{scheme}_count{model_count}",
    model_name=f"german_count2{model_count}",
    scheme = f"{scheme}"
    )
...
with open("results_massaged_german.csv", "a") as f:
