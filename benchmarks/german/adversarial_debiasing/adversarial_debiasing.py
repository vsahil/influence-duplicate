...
import load_german_credit as load_file

perm = int(sys.argv[1])
ordering = load_file.permutations(perm)
biased_test_points = np.load(f"{os.path.dirname(os.path.realpath(__file__))}/../../german/german_biased_points.npy")
debiased_test = False
...
if debiased_test:
    ...
    assert(len(dataset_orig_test.convert_to_dataframe()[0]) < 1000 - train_examples)
else:
    assert(len(dataset_orig_test.convert_to_dataframe()[0]) == 1000 - train_examples)
...
def find_discm_examples(class0_data, class1_data, print_file, scheme):
        ...
        cols = ['Checking-ccount','Months','Credit-history','Purpose','Credit-mount','Svings-ccount','Present-employment-since','Instllment-rte','Gender','Other-debtors','Present-residence-since','Property','ge','Other-instllment-plns','Housing','Number-of-existing-credits','Job','Number-of-people-being-lible','Telephone','Foreign-worker']
        df0 = pd.DataFrame(data=class0_data, columns=cols, dtype='float')
        df0['target'] = 0
        df0_binary = BinaryLabelDataset(df=df0, label_names=['target'], protected_attribute_names=['Gender'])
        df0_pred = debiased_model.predict(df0_binary)

        df1 = pd.DataFrame(data=class1_data, columns=cols, dtype='float')
        df1['target'] = 0
        df1_binary = BinaryLabelDataset(df=df1, label_names=['target'], protected_attribute_names=['Gender'])
...
# with open("results_adversarial_debiased_german.csv", "a") as f:
