...
def load_compas_two_year_nosensitive(perm=-1, debiased_test=True, validation_size=0):
	...
	if debiased_test:
		...
		biased_test_points = np.load(f"{os.path.dirname(os.path.realpath(__file__))}/compas-ground_biased_points_dist{dist}.npy")
...
def load_fair_representations(perm, training_dataset, training_labels, debiased_test=True, validation_size=0):
	...
	total_labels = pd.read_csv(f"{os.path.dirname(os.path.realpath(__file__))}/../../compas-dataset/target_groundtruth_as_label.csv").to_numpy()
	total_labels = total_labels.flatten()
	assert(perm < 20)		# we only have 20 permutations
	if perm >= 0:	# for negative number don't do
		ordering = permutations(perm)
...
def disparate_removed_load_compas(perm, debiased_test=True, validation_size=0):
	sys.path.insert(1, "../")
	sys.path.append("../../../")
	sys.path.append("../../../competitors/AIF360/")
	from aif360.datasets import MyCompasGroundDataset
...
def before_preferential_sampling(perm, validation_size=0):
	original_dataset = pd.read_csv(f"{os.path.dirname(os.path.realpath(__file__))}/../../compas-dataset/groundtruth_as_label.csv")
