...
def load_salary_nosensitive(perm=-1, debiased_test=True, validation_size=0):
	total_dataset = pd.read_csv("../../salary-dataset/normalized_salary_features.csv")
	...
	if debiased_test:
		...
		biased_test_points = np.load(f"{os.path.dirname(os.path.realpath(__file__))}/salary_biased_points_dist{dist}.npy")
...
def load_salary_partial_method1(perm, model_count, train_pts_removed, name, debiased_test=True, validation_size=0):
	total_dataset = pd.read_csv("../../salary-dataset/normalized_salary_features.csv").to_numpy()
	total_labels = pd.read_csv("../../salary-dataset/salary_labels.csv").to_numpy()
	total_labels = total_labels.flatten()
	assert(perm < 20)		# we only have 20 permutations
	...
	train_examples = 40		# testing set is 40
	X_train = total_dataset[:train_examples]
	Y_train = total_labels[:train_examples]
...
def load_salary_partial(index, perm=-1, validation_size=0):
	total_dataset = pd.read_csv("../../salary-dataset/normalized_salary_features.csv").to_numpy()
	total_labels = pd.read_csv("../../salary-dataset/salary_labels.csv").to_numpy()
	total_labels = total_labels.flatten()
	assert(perm < 20)		# we only have 20 permutations
