...
from load_salary import before_massaging_dataset, massaged_dataset, dist
...
model = Fully_Connected(
    ...
    log_dir=f'throw1/log{model_count}',
...
model_ = Fully_Connected(
    ...
    log_dir=f'throw1/log{model_count}',
    hvp_files = f"inverse_HVP_schm{scheme}_count{model_count}",
    model_name=f"salary_count2{model_count}",
    scheme = f"{scheme}"
    )
...
import sklearn, math
if len(np.unique(data_sets_final.test.x[:, sensitive_attr])) == 2:
    ...
    assert len(np.unique(data_sets_final.test.x[:, sensitive_attr])) == 1     # only one class
    class_pred = np.argmax(test_predictions, axis=1)
    class_truth = data_sets_final.test.labels
    ...
    class_fpr = fp / (fp + tn)
    class_fnr = fn / (fn + tp)
    which_class = np.unique(data_sets_final.test.x[:, sensitive_attr])[0]
    if which_class == 0:
