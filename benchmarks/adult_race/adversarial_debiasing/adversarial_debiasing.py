...
from aif360.datasets import AdultRaceDataset, BinaryLabelDataset, StructuredDataset
...
dataset_orig = AdultRaceDataset(
    ...
    privileged_classes=[[0]],       # this might be 0.  
    normalized = False,
    permute=perm 
)
...
if debiased_test:
    ...
    assert(len(dataset_orig_test.convert_to_dataframe()[0]) < 43131 - train_examples)
else:
    assert(len(dataset_orig_test.convert_to_dataframe()[0]) == 43131 - train_examples)
...
def find_discm_examples(class0_data, class1_data, print_file, scheme):
        ...
        df0['target'] = 0
        df0_binary = BinaryLabelDataset(df=df0, label_names=['target'], protected_attribute_names=['race'])
        df0_pred = debiased_model.predict(df0_binary)

        df1 = pd.DataFrame(data=class1_data, columns=cols, dtype='float')
        df1['target'] = 0
        df1_binary = BinaryLabelDataset(df=df1, label_names=['target'], protected_attribute_names=['race'])
