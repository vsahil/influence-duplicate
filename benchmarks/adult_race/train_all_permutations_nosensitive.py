...
from load_adult_race import load_adult_race_nosensitive, load_adult_race_partial_method1, dist
...
data_sets = load_adult_race_nosensitive(perm, debiased_test=debiased_test)
...
model = Fully_Connected(
    ...
    hvp_files = f"inverse_HVP_adult_race_nosensitive/inverse_HVP_schm{scheme}_count{model_count}",
    model_name=name,
    scheme = f"{scheme}")
...
data_sets2 = load_adult_race_partial_method1(perm=perm, model_count=model_count, train_pts_removed=train_pts_removed[0], name=name, debiased_test=debiased_test)
