...
def load_compas(perm=-1, validation_size=0):
	# total_dataset = genfromtxt("../../compas-dataset/normalized_compas_features.csv", delimiter=",")      # this is the standarised/normalised data, so no need to renormalize
	# total_labels = genfromtxt("../../adult-income-dataset/adult_labels.csv", delimiter=",")
	...
	train_examples = 8000		# testing set is 1884		# weird size (about 20% - similar to german credit dataset and adult income dataset)
	X_train = total_dataset[:train_examples]
	X_validation = total_dataset[train_examples:train_examples + validation_size]
	X_test  = total_dataset[train_examples + validation_size:]
	Y_train = total_labels[:train_examples]
...
def produce_permutations():
	...
	for split in range(20):
		idx = np.random.permutation(9884)	# size of compas dataset
