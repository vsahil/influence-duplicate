...
data_sets = load_student_nosensitive(perm)
...
model = Fully_Connected(
    ...
    mini_batch=True,
    train_dir=f'trained_models_german_nosensitive/output_count{model_count}', 
    log_dir=f'throw/log{model_count}',
    hvp_files = f"inverse_HVP_german_nosensitive/inverse_HVP_schm{scheme}_count{model_count}",
    model_name=name,
    scheme = f"{scheme}")
...
data_sets2 = load_student_partial_method1(perm=perm, model_count=model_count, train_pts_removed=train_pts_removed[0], name=name)
...
with open("results_student_nosensitive.csv", "a") as f:
